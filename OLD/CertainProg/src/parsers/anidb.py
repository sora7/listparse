...
__author__="alex"
__date__ ="$24.07.2014 15:40:55$"

import HTMLParser
import re
import base64

from parsers.common import AniTitle, AniList, Dict, Bool, pass_year
from parsers.common import listtype, StopPleaseException#, DBParser
...
def list_person(list_file):
    with open(list_file) as f:
        file_text = f.read()
        table_parser = Person()
        table_parser.feed(file_text.decode('utf-8'))
...
def list_company(list_file):
    with open(list_file) as f:
        file_text = f.read()
        company_parser = Company()
        company_parser.feed(file_text.decode('utf-8'))
...
def list_mylist(list_file, completed=False):
    with open(list_file) as f:
        file_text = f.read()
        mylistParser = Mylist()
        mylistParser.feed(file_text)

        titles = mylistParser.titles
        return titles

def title(title_file):
...
class AniDBParser(object):
    ...
    def list_check(self, list_file):
        return list_check(list_file)
    
    def list_person(self, list_file):
        return list_person(list_file)
    
    def list_company(self, list_file):
        return list_company(list_file)
    
    def list_mylist(self, list_file, completed=False):
        return list_mylist(list_file, completed=False)
    
    def title(self, title_file):
        return title(title_file)

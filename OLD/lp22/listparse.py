...
#defaults@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def mk_listbox(self, frame, side='top'):
        BORDER = 0
        COLOR = 'green'
    ...
    def mk_main_frame(self, main):
        if main == None:
            # standalone
            main_frame = Tkinter.Frame(master=self.root, bg='black',bd=self.BD)
    ...
    def mk_result_frame(self, main_frame):
        ...
        self.listboxes['result'] = self.mk_listbox(result_frame)
    ...
    def mk_selected_frame(self, additinal_frame):
        ...
        self.listboxes['selected'] = self.mk_listbox(selected_frame,
                                                     side='left')
...
# #            )
    ...
    def mk_awailable_frame(self, additinal_frame):
        ...
        self.listboxes['awailable'] = self.mk_listbox(awailable_frame)
...
class ListCompareModel:
    ...
    def __init__(self, view_):
        ...
        self.reload_lock2 = threading.Lock()
        self.reload_lock1 = threading.Event()
        
        self.reload_lock = False
        self.compare_lock = False
    ...
    def list_compare(self):
        if not self.compare_lock:
            self.compare_lock = True
            t1 = threading.Thread(target=self.listCompare)
            t1.start()
            #t1.join(0.1)
            #self.reloadLists()
        else:
...
##        self.reloadLists()
##        t1 = threading.Thread(target=self.reloadLists)
##        t1.start()
        
        if not self.reload_lock:
...
##        e.set()
    ...
    def reloadLists(self):
        ...
        for list_file in filter(lambda item: isfile(item), dir_items):
            lst = list_check(list_file)
            if lst.type != listtype.UNKNOWN:
                lst.path = list_file
